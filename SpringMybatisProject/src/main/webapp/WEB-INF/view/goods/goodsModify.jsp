...
<input type="hidden" name="prodName" value="${goodsCommand.prodName }">
	<table border = 1 align="center">
		<tr><th>상품 번호</th> 
			...
			<c:if test="${goodsCommand.ctgr=='cosmetic' }">selected</c:if>
			>화장품</option>
			<option value="food"
			<c:if test="${goodsCommand.ctgr=='food' }">selected</c:if>
			>음식</option>
			<option value="car"
			<c:if test="${goodsCommand.ctgr=='car' }">selected</c:if>
			>차</option>
			
			
			</select>-상품번호 : ${goodsCommand.prodNum }</td></tr>		
		<tr><th>상품명</th>
			<td><input type="text" name="prodName" 
						value="${goodsCommand.prodName }"/></td></tr>
		<tr><th>가격</th>
			<td><input type="number" min="10" step="10" name="prodPrice" 
			value="${goodsCommand.prodPrice }"
				/>
				<form:errors path="prodPrice"/></td></tr>
		<tr><th>용량</th>
			<td><input type="number" min ="10"name="prodCapacity" 
				value="${goodsCommand.prodCapacity }"/>
				<form:errors path="prodCapacity"/></td></tr>
		<tr><th>공급처</th>
			<td><input type="text" name="prodSupplyer" 
				value="${goodsCommand.prodSupplyer }"/>
				<form:errors path="prodSupplyer"/></td></tr>
		...
		<tr><th>추천여부</th>
			<td>
				...
				<c:if test="${goodsCommand.recommend.trim() == 'Y' }">checked</c:if>
				/>추천
				<input type="radio" name="recommend" value="N" 
				<c:if test="${goodsCommand.recommend.trim() == 'N' }">checked</c:if>
				/>비추
			</td></tr>
		...
		<tr><th colspan="2">
			...
			<input type="button" value="취소" 
				...
				onclick="javascript:location.href='prodDel?prodNum=${goodsCommand.prodNum }';"/> 
