...
<script>
	...
	function selectedDel(){
		var chk = document.getElementsByName("prodCk"); // 선택항목임
		var prodNums='';
		for(var i =0; i<chk.length; i++){
			if(chk[i].checked){
...
<form action="goodsBuy" method="post" onsubmit="return goodsCheck();">
	<tr><td colspan="8">
	<button type="button"  id="cartDel"
	onclick="selectedDel()" >선택항목 삭제</button></td></tr>
...
<c:forEach items="${lists }" var="dto">
	<tr><td colspan="4">
		...
		onclick="javascript:location.href='cartProdDel?prodNum=${dto.cartDTO.prodNum }';"/></td><!--get방식  --> 
		</tr>
