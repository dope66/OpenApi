...
<body>
	...
	<form action="venta.vnt" method="post" name="frm">
		아이디 : <input type="text" name="memId" /><br /> <input type="submit"
			value="검색" />
	</form>
	<table border=1>
		<tr>
		...
		<c:forEach items="${list }" var="dto">
			<tr>
				<td><c:if test="${dto.memId == null}">비회원</c:if> <c:if
						test="${dto.memId != null}">
						<a href="userSales.vnt?memId=${dto.memId}" target="_blank">${dto.memId}</a>
					</c:if></td>
				<td><c:if test="${dto.memName == null}">비회원</c:if> <c:if
						test="${dto.memName != null}">${dto.memName}</c:if></td>
				<td>${dto.prodName }</td>
				<td>${dto.purchaseQty }</td>
