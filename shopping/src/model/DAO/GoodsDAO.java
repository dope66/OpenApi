...
public class GoodsDAO extends DataBaseInfo {
	final String COLUMNS = "PROD_NUM, PROD_NAME, PROD_PRICE," + "PROD_IMAGE, PROD_DETAIL,PROD_CAPACITY,PRUD_SUPPLYER,"
	...
	public List<OrderList> orderlist(String memId) {
		...
		sql = " select p2.purchase_date , p4.PAYMENT_APPR_NUM, p1.prod_num, "
				...
				+ " where   p2.PUrCHASE_NUM =p3.PUrCHASE_NUM and p1.prod_num= p3.prod_num and "
				+ " 		p2.PUrCHASE_NUM=p4.PUrCHASE_NUM(+) and p2.mem_id= ?"
	...
	public void purchaseInsert(PurchaseDTO dto) {
		sql = " insert into purchase (PURCHASE_NUM,MEM_ID," + " PURCHASE_TOT_PRICE, PURCHASE_ADDR, PURCHASE_METHOD,"
	...
	public ProductCartDTO prodCart(String prodNum, String memId) {
		...
		sql = "select p.PROD_NUM , PROD_NAME, PROD_PRICE, " + "	  PRUD_SUPPLYER, PROD_DEL_FEE, prod_image,"
				+ "       MEM_ID, CART_QTY, CART_PRICE" + " from products p, cart c" + " where p.PROD_NUM = c.PROD_NUM "
	...
	public List cartList(String memId) {
		...
		sql = " select p.PROD_NUM , PRUD_SUPPLYER, PROD_DEL_FEE," + "  PROD_IMAGE, PROD_NAME ,PROD_PRICE,"
				+ "        CART_PRICE, CART_QTY" + " from products p, cart c"
	...
	public void cartInsert(CartDTO dto) {
		sql = " merge into cart c " + " using (select prod_num " + "        from products where prod_num = ?) p "
				...
				+ " When not MATCHED THEN  " + " insert (c.MEM_ID,c.PROD_NUM,c.CART_QTY," + "c.CART_PRICE) "
				+ " values(?,?,?,?)";
		getConnect();
		try {
			pstmt = conn.prepareStatement(sql);
	...
	public void goodsUpdate(ProductDTO dto) {
		sql = " update products " + " set PROD_NAME = ? , PROD_PRICE = ?," + "     PROD_DETAIL = ?, PROD_CAPACITY= ? , "
				+ "     PRUD_SUPPLYER = ? , PROD_DEL_FEE = ?," + "     RECOMMEND = ?" + " where PROD_NUM = ? ";
		getConnect();
		try {
			pstmt = conn.prepareStatement(sql);
